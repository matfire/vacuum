...
program
  ...
  .action((grid: string, position: string, direction: direction) => {
    const vacuum = new Vacuum(
      grid.split(",").map((e: string) => parseInt(e)),
      position.split(",").map((e: string) => parseInt(e)),
      direction
    );
    ...
    r1.question("Enter instructions: ", (instructions) => {
      ...
      console.log(
        `x=${vacuum.getPosition()[0]} y=${
          vacuum.getPosition()[1]
